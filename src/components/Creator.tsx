...
export default function Creator({ locale }: Props) {
  ...
  return <section className="maker" id="create">
    ...
    <div className="maker-grid"><form className="maker-form" onSubmit={submit}>
      <div className="kind-toggle" role="group" aria-label={t.steps[0]}><button type="button" className={kind === 'blessing' ? 'selected' : ''} onClick={() => setKind('blessing')}>{t.blessing}</button><button type="button" className={kind === 'wish' ? 'selected' : ''} onClick={() => setKind('wish')}>{t.wish}</button></div>
      ...
      <details className="advanced" open={advancedOpen} onToggle={(event) => setAdvancedOpen(event.currentTarget.open)}><summary>{t.personal} <i>+</i></summary><div><label className="editor-field"><span>{kind === 'wish' ? t.wishMind : t.detail}</span><textarea name="note" required={kind === 'wish'} placeholder={kind === 'wish' ? t.wishPlaceholder : t.detailPlaceholder} /></label><label className="editor-field"><span>{t.name}</span><input name="name" placeholder={t.namePlaceholder} /></label><input type="hidden" name="length" value="medium" /></div></details>
